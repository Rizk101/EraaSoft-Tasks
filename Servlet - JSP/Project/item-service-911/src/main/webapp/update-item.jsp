...
<% if (session.getAttribute(UserController.LOGGED_IN_USER) == null) { response.sendRedirect(request.getContextPath() + "/login.jsp"); return; }
   if (request.getAttribute("itemData") == null) { response.sendRedirect(request.getContextPath() + "/ItemController?action=showItems"); return; } %>
<!DOCTYPE html>
<html lang="en" >
<head>
  <meta charset="UTF-8">
  ...
  <form action="<%= request.getContextPath() %>/ItemController" method="post">
    <div class="form-row">
      <div class="input-data">
        <input type="text" name="name" required maxlength="255" value="<%= escapeHtml(item.getName()) %>">
        <div class="underline"></div>
        <label>Name</label>
      </div>
      
      <div class="input-data">
        <input type="number" required name="price" min="0.01" step="0.01" value="<%=item.getPrice() %>">
        <div class="underline"></div>
        <label>PRICE</label>
      </div>
    ...
    <div class="form-row">
      <div class="input-data">
        <input type="number" required name="totalNumber" min="0" step="1" value="<%=item.getTotalNumber() %>">
        <div class="underline"></div>
