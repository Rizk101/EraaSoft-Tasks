...
<%
    ...
    if (item == null || itemDetails == null) { response.sendRedirect(request.getContextPath() + "/ItemController?action=showItems"); return; }
%>
<!DOCTYPE html>
<html lang="en"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"><title>Update Item Details</title>
...
<body><main class="container"><h1>Update Item Details</h1><p class="item"><%= escapeHtml(item.getName()) %></p><form action="<%= request.getContextPath() %>/ItemDetailsController" method="post"><input type="hidden" name="action" value="update"><input type="hidden" name="itemId" value="<%= item.getId() %>"><label>Description</label><textarea name="description" required maxlength="1000"><%= escapeHtml(itemDetails.getDescription()) %></textarea><label>Category</label><input type="text" name="category" required maxlength="100" value="<%= escapeHtml(itemDetails.getCategory()) %>"><button type="submit">Update Item Details</button></form><p class="back"><a href="<%= request.getContextPath() %>/ItemController?action=showItems">Back To Items</a></p></main></body></html>
