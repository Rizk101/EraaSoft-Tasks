...
%>
<% if (session.getAttribute(UserController.LOGGED_IN_USER) != null) { response.sendRedirect(request.getContextPath() + "/ItemController?action=showItems"); return; } String message = request.getParameter("message"); %>
<!DOCTYPE html>
<html lang="en"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"><title>Forgot Password</title>
...
<body><main class="container"><h1>Reset Password</h1><% if (message != null) { %><p class="message"><%= escapeHtml(message) %></p><% } %><form action="<%= request.getContextPath() %>/UserController" method="post"><input type="hidden" name="action" value="forgotPassword"><label>Email</label><input type="email" name="email" required><label>New Password</label><input type="password" name="password" required minlength="8"><button type="submit">Reset Password</button></form><p class="link"><a href="<%= request.getContextPath() %>/login.jsp">Back to Login</a></p></main></body></html>
